...
const { retry, exec } = require('../utils.js')
...
async function wifiSetup(info) {
  ...
  try {
    ...
    await retry(() => request(`http://${gateway_ip}/config?ssid=${info.ssid}&pass=${info.pass}&blynk=${info.auth}&host=${info.server}&port=${info.port}`), 5, 1000)
  } finally {
    debug('Disconnecting...');
    await exec(`nmcli connection delete "${info.product}"`, true)
  }
