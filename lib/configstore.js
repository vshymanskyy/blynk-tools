...
let configUser;
try {
  if (!fs.existsSync(configUserFn)) {
    fs.writeJsonSync(configUserFn, {});
    console.log(`Created new config file: ${configUserFn}`)
  }
  debug(`Loading ${configUserFn}`)
  configUser = tryParse(fs.readFileSync(configUserFn))

  if (!_.isPlainObject(configUser)) {
